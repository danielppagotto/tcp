...
set.seed(2024)
amostra <- sample_n(data_filtro, 200)
```
...
kmeans_result <- kmeans(mds_df_kmeans[, c("Dim1", "Dim2")], centers=4)  # K-means nos dados MDS
...
```{r}
# 1. Aplique o k-means diretamente ao data frame mds_df_kmeans
set.seed(2024)
agrup_kmeans_4 <- kmeans(dissim, centers=4)

# 2. Calcule a silhueta usando as distâncias euclidianas no espaço do MDS
silhueta_3 <- silhouette(agrup_kmeans_4$cluster, dist(dissim))

# 3. Calcule a média da largura da silhueta (terceira coluna)
media_silhueta_kmeans_4 <- mean(silhueta_3[, 3])

# Exiba o resultado
media_silhueta_kmeans_4
...
```{r}
# 1. Aplique o método PAM diretamente ao data frame mds_df_pam
set.seed(2024)
agrup_pam_4 <- pam(dissim, k = 4)

# 2. Calcule a silhueta usando as distâncias euclidianas no espaço do MDS
silhueta_pam_4 <- silhouette(agrup_pam_4$clustering, dist(dissim))

# 3. Calcule a média da largura da silhueta (terceira coluna)
media_silhueta_pam_4 <- mean(silhueta_pam_4[, 3])

# Exiba o resultado
media_silhueta_pam_4
```

```{r}
eps <- 20  # Ajuste conforme necessário
minPts <- 5  # Ajuste conforme necessário

dbscan_result <- dbscan(dissim, eps = eps, minPts = minPts)

# 3. Visualize os resultados
# Se você quiser visualizar os clusters, você pode usar MDS para reduzir a dimensionalidade
mds <- cmdscale(dissim, k=2)  # Reduzindo para duas dimensões
mds_df <- as.data.frame(mds)
mds_df$cluster <- as.factor(dbscan_result$cluster)

# 4. Gere o gráfico com os clusters
library(ggplot2)

ggplot(mds_df, aes(x=V1, y=V2, color=cluster)) +
  geom_point(size=3) +
  labs(title="DBSCAN Clustering", x="Dimensão 1", y="Dimensão 2") +
  theme_minimal()
```
