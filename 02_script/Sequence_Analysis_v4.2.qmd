...
Observa-se que muitos valores "null"foram gerados.. Os valores "null" referem-se a indivíduos que não completaram as 5 entrevistas entre 2018 e 2019, ou aqueles que deixaram de responder alguma das sessões (esses valores totalizaram 112.709 indivíduos).
...
Agora que temos a base tratada, iremos utiliza-la para gerar os gráficos do Sequence Analysis. Primeiramente, iremos definir as variáveis temporais na função wcAggregateCases, para que as demais funções reconheça as variáveis temporais.
...
png("distribuicao_estados_pre_pandemia.png", width = 1200, height = 800, res = 150)
#Exibindo as frequências relativas dos grupos
seqdplot(mvad.seq, with.legend = "right", border = NA, main = "Distribução de estados - Pré-pandemia")
```

```{r}

png("grafico_índices_pre_pandemia.png", width = 1200, height = 800, res = 150)
#Exibindo todas as sequências do dataframe (gráfico de índices)
seqIplot(mvad.seq, sortv = "from.start", with.legend = "right", main = "Gráfico de Índices - Pré-pandemia")
```
...
seq_heatmap(mvad.seq, agnes)
```

A seguir, serão utilizados dois métodos distintos para definir o número de clusters. O primeiro método usa a função `cutree` para segmentar os dados com base na clusterização gerada pela função `agnes`. O segundo método emprega a função `wcKMedoids` para realizar a clusterização utilizando o algoritmo K-Medoids. Por fim, será gerado um gráfico mostrando a frequência relativa, os índices, e as sequências mais representativas para cada cluster definido.
...
seqdplot(mvad.seq, group=pamClustRange$clustering$cluster5, xtlab=1:5, border=NA,main = "Cluster da distribuição dos estados-novo" ,cex.legend=0.9)
dev.off()
...
seqfplot(mvad.seq, group=pamClustRange$clustering$cluster5, xtlab=1:5, border=NA,main = "Cluster da distribuição dos estados-novo" ,cex.legend=0.9)
dev.off()
...
Observa-se que muitos valores "null". Os valores "null" referem-se a indivíduos que não completaram as 5 entrevistas entre 2020 e 2021, ou aqueles que deixaram de responder alguma das sessões (esses valores totalizaram 86.564 indivíduos).
...
```{r}
#Definindo as variáveis temporais, ou os "Estados"
aggMvad <- wcAggregateCases(data_pandemia_wide[, 2:6])
print(aggMvad)
uniqueMvad <- data_pandemia_wide[aggMvad$aggIndex, 2:6]
mvad.seq <- seqdef(uniqueMvad, weights = aggMvad$aggWeights)
...
```{r}
png("distribuicao_estados_pandemia.png", width = 1200, height = 800, res = 150)

#Exibindo as frequências relativas dos grupos
seqdplot(mvad.seq, with.legend = "right", border = NA, main = "Distribução de estados - Período pandêmico")
```

```{r}

png("grafico_indices_pandemia.png", width = 1200, height = 800, res = 150)
#Exibindo todas as sequências do dataframe (gráfico de índices)
seqIplot(mvad.seq, sortv = "from.start", with.legend = "right", main = "Gráfico de Índices - Período pandêmico")
```
...
count_tcp <- data_pre_pandemia_wide |> 
  filter(rowSums(across(2:6, ~.x == "CP", .names = "col_{.col}")) == 5) |> 
  count()
...
#Inserindo a variável de cluster
uniqueCluster5  <-  pamClustRange$clustering$cluster5 
data_pandemia_wide$cluster5  <- uniqueCluster5[aggMvad$disaggIndex] 
...
Observa-se que muitos valores "null". Os valores "null" referem-se a indivíduos que não completaram as 5 entrevistas entre 2022 e 2023, ou aqueles que deixaram de responder alguma das sessões (esses valores totalizaram 37.737 indivíduos).
...
```{r}
#Definindo as variáveis temporais, ou os "Estados"
aggMvad <- wcAggregateCases(data_pos_pandemia_wide[, 2:6])
print(aggMvad)
uniqueMvad <- data_pos_pandemia_wide[aggMvad$aggIndex, 2:6]
mvad.seq <- seqdef(uniqueMvad, weights = aggMvad$aggWeights)
...
png("distribuicao_estados_pos_pandemia.png", width = 1200, height = 800, res = 150)
...
png("grafico_indices_pos_pandemia.png", width = 1200, height = 800, res = 150)
#Exibindo todas as sequências do dataframe (gráfico de índices)
...
count_tcp <- data_pos_pandemia_wide |> 
  filter(rowSums(across(2:6, ~.x == "CP", .names = "col_{.col}")) == 5) |> 
  count()

count_tcp_1 <- data_pos_pandemia_wide |> 
  filter(`1` == "CP") |> 
  count()

count_tcp_5 <- data_pos_pandemia_wide |> 
  filter(`5` == "CP") |> 
  count()

count_pfft_1 <- data_pos_pandemia_wide |> 
  filter(`1` == "PFFT") |> 
  count()

count_pfft_5 <- data_pos_pandemia_wide |> 
  filter(`5` == "PFFT") |> 
  count()

count_espr_1 <- data_pos_pandemia_wide |> 
  filter(`1` == "ESPR") |> 
  count()

count_espr_5 <- data_pos_pandemia_wide |> 
  filter(`5` == "ESPR") |> 
  count()

count_emp_1 <- data_pos_pandemia_wide |> 
  filter(`1` == "EMP") |> 
  count()

count_emp_5 <- data_pos_pandemia_wide |> 
  filter(`5` == "EMP") |> 
  count()

count_deso_1 <- data_pos_pandemia_wide |> 
  filter(`1` == "Desocupado") |> 
  count()

count_deso_5 <- data_pos_pandemia_wide |> 
  filter(`5` == "Desocupado") |> 
  count()

count_espu_1 <- data_pos_pandemia_wide |> 
  filter(`1` == "ESPU") |> 
  count()

count_espu_5 <- data_pos_pandemia_wide |> 
  filter(`5` == "ESPU") |> 
  count()

count_tfnr_1 <- data_pos_pandemia_wide |> 
  filter(`1` == "TFNR") |> 
  count()

count_tfnr_5 <- data_pos_pandemia_wide |> 
  filter(`5` == "TFNR") |> 
  count()
```
...
pamClustRange <- wcKMedRange(dissim, kvals = 4, weights = aggMvad$aggWeights)
#Inserindo a variável de cluster
uniqueCluster5  <-  pamClustRange$clustering$cluster5 
data_pos_pandemia_wide$cluster5  <- uniqueCluster5[aggMvad$disaggIndex]
...
seqIplot(mvad.seq, group=part_1, xtlab=1:5, border=NA,main = "Cluster da distribuição dos estados-novo" ,cex.legend=0.9)
dev.off()
