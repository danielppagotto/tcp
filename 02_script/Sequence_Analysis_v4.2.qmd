...
png("grafico_frequencia_pos_pandemia.png", width = 1200, height = 800, res = 150)
#Exibindo as 10 sequências mais frequentes
seqfplot(mvad.seq, with.legend = "right", idxs = 2:7)
...
#Contagem de transições entre CP e PFFT
count_tcp_PFFT <- data_pos_pandemia_wide |> 
  filter(rowSums(across(2:6, ~ .x == "CP")) > 0 & 
         rowSums(across(2:6, ~ .x == "PFFT")) > 0) |> 
  count()

#Contagem de transições entre CP e ESPR
count_tcp_espr <- data_pos_pandemia_wide |> 
  filter(rowSums(across(2:6, ~ .x == "CP")) > 0 & 
         rowSums(across(2:6, ~ .x == "ESPR")) > 0) |> 
  count()
```
