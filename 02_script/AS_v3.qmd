...
Analisar quais setores/atividades estão associados a cada padrão de carreira gera um nível de complexidade em que as funções do `survey` não são capazes de lidar, e por isso utilizamos as do `tidyverse` considerando os pesos populacionais.

A análise setorial é realizada apenas considerando a ocupação de trabalho por conta própria, mas ao considerar que a
...
descritivo_atividade <- dados_tratados |> 
  ...
  summarise(freq = sum(V1028), .groups = "drop") %>%  # aqui entra o peso
  group_by(cluster) %>%
