...
### 1.5.1.3. Situação do domicílio

```{r}
urbanidade <- dados_cluster |> 
  group_by(V1022, cluster) |> 
  summarise(total = sum(V1028), .groups = "drop") |>
  pivot_wider(names_from = cluster, values_from = total) |> 
  mutate(perc_Persistente = Persistente / (Persistente + Precarizado + Transitório),
         perc_Precarizado = Precarizado / (Persistente + Precarizado + Transitório),
...
### 1.5.1.4. Quantidade de pessoas no domicílio

```{r}
qtd_pessoas <- dados_cluster %>%
    group_by(cluster) |> 
    summarise(across(pessoas_domicilio, list(
...
### 1.5.1.9. Atividade profissional
...
#### 1.5.1.10.1. Renda média por região
