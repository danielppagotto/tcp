...
data_0 <- read_csv("C:/Users/alefs/OneDrive/Documentos/LAPEI-CIGETS/GitHub/tcp/01_dados/as_2023_2024_v4.csv") |> 
...
```{r}
silhoutte_1 <- fviz_nbclust(
  mds_df_kmeans,
  kmeans,
  method = "silhouette",
  k.max = 7)

ggsave(filename = "silhoutte_1.png",
       plot = silhoutte_1,
       width = 8,
       height = 6,
       dpi = 300)
...
indice_cluster <- seqIplot(seq, 
    ...
    use.layout = TRUE,
    legend.prop = 0.15,
    main = c("Persistentes", "Necessidade", "Intermitente"))
...
Abaixo, definiremos o objeto `design` que irá incorporar os pesos populacionais. E iremos transformar as variáveis categóricas em dummy binárias, para podermos aplicar os testes de diferenças.

```{r}
design <- svydesign(ids = ~UPA, weights = ~V1028, data = dados_cluster)

#Binarizando a variável chefe_familia
design$variables$chefe_domicilio <- ifelse(design$variables$chefe_domicilio == "Responsável pelo domicílio", 1, 0)

#Binarizando a variável ensino_superior
design$variables$ensino_superior <- ifelse(design$variables$ensino_superior == "Superior completo", 1, 0)
```

Os testes de diferenças serão aplicados exclusivamente nas variáveis que apresentarem resultados descritivos próximos. Primeiro, aplicaremos o teste de independência de Rao & Scott (variáveis categóricas/dummy) e o teste T Student (variáveis numéricas), para cada combinação possíveis de pares dos clusters. E depois, aplicaremos os testes post-hoc para corrigir a taxa de erro familiar, e obter, os p-valores ajustados para observar diferenças significativas.

```{r}

#Definindo os pares
clusters <- unique(design$variables$cluster)
pares <- combn(clusters, 2, simplify = FALSE)

#Definindo função para o teste de Rao & Scott
posthoc_chi <- function(var){
  
  teste_chi_par <- function(par){
    sub_d <- subset(design, cluster %in% par)
    
    f <- as.formula(paste("~", var, "+ cluster"))
    
    out <- tryCatch(
      svychisq(f, sub_d, statistic = "F"),
      error = function(e) return(list(statistic = NA, p.value = NA))
    )
    
    tibble(
      par = paste(par[1], "vs", par[2]),
      estatistica_f = as.numeric(out$statistic),
      p_valor = as.numeric(out$p.value)
    )
  }
  
  map_dfr(pares, teste_chi_par) |> 
    mutate(
      variavel = var,
      p_bonferroni = p.adjust(p_valor, method = "bonferroni"),
      p_holm = p.adjust(p_valor, method = "holm"),
      p_fdr = p.adjust(p_valor, method = "fdr"),
      significativo = p_bonferroni < 0.05
    )
}

#Definindo função para o teste T Student

posthoc_ttest <- function(var){
  
  teste_t_par <- function(par){
    ...
    out <- tryCatch(
      svyttest(f, sub_d),
      error = function(e) return(list(p.value = NA, statistic = NA,
                                      estimate = NA))
    )
    
    dif <- if(is.null(out$estimate)) NA else -out$estimate[[1]]
    
    tibble(
      par = paste(par[1], "vs", par[2]),
      diferenca_media = dif,
      t_valor = as.numeric(out$statistic),
      p_valor = as.numeric(out$p.value)
    )
  }
  
  map_dfr(pares, teste_t_par) |> 
    mutate(
      variavel = var,
      p_bonferroni = p.adjust(p_valor, method = "bonferroni"),
      p_holm = p.adjust(p_valor, method = "holm"),
...
diferencas_urbanidade <- posthoc_chi("V1022")
```
...
diferencas_chefe_familia <- posthoc_chi("chefe_domicilio")
...
```{r}
diferencas_genero <- posthoc_chi("V2007")
```
...
sub_design <- update(design,
                     is_branco = ifelse(V2010 == "Branca",1,0))

teste_rao_scott <- svychisq(~is_branco + cluster,
                            design = sub_design,
                            statistic = "F")

print(teste_rao_scott)

diferenca_raca <- posthoc_chi("is_branco")
```
...
descritivo_atividade <- dados_tratados |> 
  ...
  mutate(
    VD4010 = case_when(
      grepl("^Agricultura", VD4010) ~ "Agricultura",
      grepl("^(Construção|Indústria geral)", VD4010) ~ "Indústria",
      grepl("^(Comércio)", VD4010) ~ "Comércio",
      grepl("^(Outros Serviços|Informação, comunicação|Transporte|Alojamento|Educação)", VD4010) ~ "Serviços",
      TRUE ~ NA_character_
    )
  ) %>% 
  left_join(id_cluster, by = "id") |> 
...
diferencas_renda <- posthoc_ttest("renda_media")
```

#### 1.5.11.1. Renda média por região
...
## 1.6. Correlação de IDH e cluster por Necessidade

#Subseção descontinuada

```{r}

cluster_necessidade_uf <- descritivo_regiao |> 
  mutate(
    UF = case_when(
      UF == "Acre" ~ "AC",
      UF == "Alagoas" ~ "AL",
...
tabela_correlacao <- cluster_necessidade_uf |> 
  left_join(idh, by = c("UF" = "uf")) |> 
    mutate(regiao = case_when(
...
test_correlacao <- cor.test(tabela_correlacao$IDHM,
                            tabela_correlacao$perc_Precarizado,
                            alternative = "less",
                            method = "kendall")
test_correlacao
...
grafico_correlacao <- tabela_correlacao |> 
  ggplot(aes(x = IDHM, y = perc_Precarizado)) +
  geom_point() +
  ...
  ylab("Percentual de TCP por sobrevivência") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  theme(
...
## 1.7. Validando amostra 2

```{r}
#|warning: FALSE
# Objeto de sequência

rownames(amostra2) <- amostra2$id_seq
...
base_join <- cbind(seq, 
                   amostra2$id_seq, 
                   amostra2$id) |> 
              rename(id_seq_original = `amostra2$id_seq`,
                     id = `amostra2$id`) |> 
              mutate(id_pos_seq = row_number())
...
```{r}
silhoutte_2 <- fviz_nbclust(
  mds_df_kmeans,
  kmeans,
  method = "silhouette",
  k.max = 7)

ggsave(filename = "silhoutte_2.png",
       plot = silhoutte_2,
       width = 8,
       height = 6,
       dpi = 300)
...
indice_cluster <- seqIplot(seq, 
    ...
    use.layout = TRUE,
    legend.prop = 0.15,
    ylab = c("1.226 seq. (20%)", "1.477 seq. (25%)", "3.297 seq. (55%)"),
    main = c("Intermitente", "Necessidade", "Persistente"))

indice_cluster
```

```{r}
base_cluster_2 <- cbind(seq, 
                   amostra1$id_seq, 
                   amostra1$id,
                   mds_df_kmeans$cluster) %>%
  rename(id_sequenciamento = `amostra1$id_seq`,
         ...
         cluster = `mds_df_kmeans$cluster`)  |> 
  mutate(cluster = as.character(cluster),
         cluster = case_when(cluster == "1" ~ "Transitório",
                              cluster == "2" ~ "Precarizado",
                              cluster == "3" ~ "Persistente"))
```

```{r}
png("indice_cluster_amostra2.png", width = 1500, height = 1200, res = 200) # largura, altura e resolução
windowsFonts(Times = windowsFont("Times New Roman"))
par(family = "Times")
indice_cluster <- seqIplot(seq, 
    group = kmeans_result$cluster,
...
rownames(amostra3) <- amostra3$id_seq
...
base_join <- cbind(seq, 
                   amostra3$id_seq, 
                   amostra3$id) |> 
              rename(id_seq_original = `amostra3$id_seq`,
                     id = `amostra3$id`) |> 
              mutate(id_pos_seq = row_number())
...
```{r}
silhoutte_3 <- fviz_nbclust(
  mds_df_kmeans,
  kmeans,
  method = "silhouette",
  k.max = 7)

ggsave(filename = "silhoutte_3.png",
       plot = silhoutte_3,
       width = 8,
       height = 6,
       dpi = 300)
...
indice_cluster <- seqIplot(seq, 
    ...
    use.layout = TRUE,
    legend.prop = 0.15,
    ylab = c("1.265 seq. (21,1%)", "3.267 seq. (55,4%)", "1.468 seq. (24,5%)"),
    main = c("Intermitente", "Persistente", "Necessidade"))

indice_cluster
```

```{r}
base_cluster_3 <- cbind(seq, 
                   amostra1$id_seq, 
                   amostra1$id,
                   mds_df_kmeans$cluster) %>%
  rename(id_sequenciamento = `amostra1$id_seq`,
  ...
  mutate(cluster = as.character(cluster),
         cluster = case_when(cluster == "1" ~ "Transitório",
                              cluster == "2" ~ "Persistente",
                              cluster == "3" ~ "Precarizado"))
```

```{r}
png("indice_cluster_amostra3.png", width = 1500, height = 1200, res = 200) # largura, altura e resolução
windowsFonts(Times = windowsFont("Times New Roman"))
par(family = "Times")
indice_cluster <- seqIplot(seq, 
    group = kmeans_result$cluster,
    border = NA, 
    ltext = c("Empregado do setor privado", "Fora da força de trabalho ou desocupado", "Trabalhador por conta própria"),
    yaxis = FALSE,
    use.layout = TRUE,
    legend.prop = 0.15,
    ylab = c("1.265 seq. (21,1%)", "3.267 seq. (55,4%)", "1.468 seq. (24,5%)"),
...
## 1.9. Testando diferenças entre as amostras

```{r}

amostras_teste <- bind_rows("amostra1" = base_cluster |>  select(cluster),
                            "amostra2" = base_cluster_2 |>  select(cluster),
                            "amostra3" = base_cluster_3 |>  select(cluster),
                            .id = "amostra")

tabela_quiquadrado <- table(amostras_teste$amostra,
                            amostras_teste$cluster)

teste <- chisq.test(tabela_quiquadrado)
